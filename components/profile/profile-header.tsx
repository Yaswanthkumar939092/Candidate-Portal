...
const LIFECYCLE_CONFIG: Record<
  ...
  { label: string; banner: string; avatarBg: string; dot: string; badge: string }
> = {
  candidate: {
    label: "Candidate",
    ...
    dot: "bg-blue-500",
    badge: "border-blue-200 bg-blue-50 text-blue-700 dark:border-blue-800 dark:bg-blue-950/60 dark:text-blue-300",
  },
  applicant: {
    label: "Applicant",
  ...
  offered: {
    ...
    dot: "bg-purple-500",
    badge: "border-purple-200 bg-purple-50 text-purple-700 dark:border-purple-800 dark:bg-purple-950/60 dark:text-purple-300",
  },
  onboarding: {
    label: "Onboarding",
    ...
    dot: "bg-orange-500",
    badge: "border-orange-200 bg-orange-50 text-orange-700 dark:border-orange-800 dark:bg-orange-950/60 dark:text-orange-300",
  },
  employee: {
    label: "Employee",
...
export function ProfileHeader({ profile, className }: ProfileHeaderProps) {
  ...
  return (
    ...
    >
      {/* ── Gradient banner ── */}
      <div className={cn("h-24 w-full bg-gradient-to-r", cfg?.banner)}>
        <div
            ...
            profile?.skills && profile?.skills.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-3">
                {profile?.skills?.map((skill) => (
                  <span key={skill} className="inline-flex items-center gap-1.5 rounded-full bg-muted px-3 py-1 text-sm text-muted-foreground">
                    {skill}
                  </span>
                ))}
              </div>
