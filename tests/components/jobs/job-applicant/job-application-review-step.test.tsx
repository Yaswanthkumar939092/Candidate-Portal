...
describe('JobApplicationReviewStep', () => {
      ...
      sections: [
        {
          ...
          fields: [
            { fieldname: 'full_name', label: 'Full Name', fieldtype: 'Data', hidden: 0 },
            { fieldname: 'email', label: 'Email Address', fieldtype: 'Data', hidden: 0 },
            { fieldname: 'hidden_field', label: 'Hidden', fieldtype: 'Data', hidden: 1 },
          ],
        },
      ],
    },
  ...
  it('renders step tabs, toggle collapse/expand, and displays dynamic summaries', () => {
    ...
    // No warning shown
    expect(screen.queryByText('Please complete all steps before submitting.')).toBeNull()
    ...
    const personalInfoTabHeader = screen.getByRole('button', { name: /Personal Info/ })
  ...
  it('displays correct summary when table field has no items added', () => {
    ...
    const workHistoryTabHeader = screen.getByRole('button', { name: /Work History/ })
  ...
  it('performs successful submission flow and calls deleteDraft when draftName is provided', async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockPush).toHaveBeenCalledWith('/open-jobs/job-123/apply-job/thank-you')
    })
  })
